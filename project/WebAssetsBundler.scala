...
import com.typesafe.sbt.web.{incremental, PathMapping}
...
import sbt.{sbtUnchecked, taskKey, AutoPlugin, File, Setting}
import sbt.internal.util.ManagedLogger
import sbt.Keys.streams

/**
  * Custom plugin to compile and bundle TypeScript and Sass using Webpack. We are not using
  * sbt-typescript https://github.com/joost-de-vries/sbt-typescript plugin because it hasn't been
  * updated since 2018 and uses Typescript 2 while current Typescript version is 4.
  *
  * Our custom plugin invokes `tsc` compiler directly through the Webpack ts-loader. `tsconfig.json`
  * and `webpack.config.js` are set up to compile code from `app/assets/javascripts` and
  * 'node_modules/@uswds/uswds/dist/js' directories and output the result to `target/web/dist`
  * folder. We also use Webpack to compile the USWDS Sass into CSS.
  *
  * This plugin introduces `bundleWebAssets` task which will be used as a pipeline stage. Read about
  * Asset Pipeline tasks https://github.com/sbt/sbt-web#asset-pipeline The task implements Asset
  * Pipeline task and not Source File task because the latter doesn't allow us to keep directory
  * structure: we want that file `assets/javascripts/foo.ts` be compiled to
  * `assets/javascripts/foo.js`. So we are using Pipeline.Stage for that.
  *
  * This plugin is enabled by adding `bundleWebAssets` as first task in pipelineStages in build.sbt.
  */
object WebAssetsBundler extends AutoPlugin {
  ...
  object autoImport {
    ...
    val bundleWebAssets =
      taskKey[Pipeline.Stage](
        "Uses Webpack to compile and bundle TypeScript, custom Sass files, and the USWDS Sass."
      )
  ...
  /**
    * Given list of all assets (css, image, TS, Sass files) figures out whether any TS or Sass files
    * have been changed and recompiles them. If no files have been changed assumes JS and CSS files
    * have been compiled at earlier iterations and returns them.
    *
    * @param inputFiles
  ...
  def recompileIfFilesChanged(
    ...
    val fileHasher: OpInputHasher[File] =
      OpInputHasher[File](f => OpInputHash.hashString(f.getCanonicalPath + f.lastModified))
    // we only watch for changes in TS files.
    val changedFiles = inputFiles
      .map(p => p._1)
      .filter { f =>
